		...
		try {
			...
			case "READ":
				try {
					...
					// since the underlying queue is a BlockingQueue, this
					// method call will wait if the queue is empty
				...
				break;
			case "READ-FROM-FILE_CONSUMER_Q":
				try {
					String content = Queue.getInstance().getFromFileConsumerQ();
					sendMessageToClient(new Message("ACK", content));
				} catch (InterruptedException e) {
					e.printStackTrace();
					sendMessageToClient(new Message("ERR", null));
